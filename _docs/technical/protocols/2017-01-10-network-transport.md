...
This guide is for developers who want to build their own client for Cryptokami SL.
Please read [Cryptokami SL Implementation Overview](/technical/) for more info.
This guide covers the network transport layer used in Cryptokami SL nodes.
...
-   **Reporting of network failures**. Network failures are not hidden from the
    ...
    transport layer should notify the application layer. In Cryptokami SL, the
...
In Cryptokami SL, multiple lightweight connections are used to support the
...
and this is what the `LOCAL_ID` refers to. Cryptokami SL, however, does not
...
unidirectional lightweight connections. Cryptokami SL follows this pattern. Refer
...
the Cryptokami SL protocol, the local endpoint ID is always 0.
