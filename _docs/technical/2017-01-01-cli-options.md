---
layout: default
title:  Cryptokami SL CLI Options
...
Cryptokami SL CLI Options
----------------------

This guide describes all executable files that are used in Cryptokami SL and all corresponding CLI-options/parameters.
...
Usage: cryptokami-node [--version] [--db-path FILEPATH] [--rebuild-db]
                    ...
                    [--flush-wallet-db]
  Cryptokami SL main server node w/ wallet.
...
Usage: cryptokami-keygen [--version] COMMAND [--configuration-file FILEPATH]
...
## cryptokami-explorer-swagger

~~~
Cryptokami SL Explorer web API docs generator.

Usage: cryptokami-explorer-swagger [--version]
...
This program runs during 'cryptokami-sl' building on Travis CI. Generated file
'explorer-web-api-swagger.json' will be used to produce HTML documentation. This
documentation will be published at cryptokamidocs.com using
...
Usage: cryptokami-node-simple [--version] [--db-path FILEPATH] [--rebuild-db]
                           [--genesis-secret INT] [--keyfile FILEPATH]
                           [--backup-phrase PHRASE] [--topology FILEPATH]
                           [--kademlia FILEPATH] [--node-id NODE_ID]
                           [--default-port PORT] [--policies FILEPATH]
                           ...
                           [--behavior FILE]
  Cryptokami SL main server node.
...
~~~
Tool to launch Cryptokami SL.

Usage: cryptokami-launcher [--version] --node PATH [-n ARG]
...
Command example:
  ...
  stack exec -- cryptokami-launcher                                   \
    --node binaries_v000/cryptokami-node                              \
...
## cryptokami-block-gen

~~~
Cryptokami SL blockchain generator

Usage: cryptokami-block-gen [--version] --blocks INT --nodes INT
...
Command example:
  ...
  stack exec -- cryptokami-block-gen           \
    --blocks 5000                           \
    --nodes 3                               \
    --coins 100                             \
...
~~~
CryptokamiSL report server

Usage: cryptokami-report-server [-p|--port INTEGER] [--logsdir FILEPATH]
                             [--severity SEVERITY] [--size-limit BYTES]
                             [--version]
  CryptokamiSL reporting server daemon
...
## cryptokami-dht-keygen

~~~
Generator of random key for Kademlia DHT.

Usage: cryptokami-dht-keygen [--version] (-n|--nonce STRING)
...
## cryptokami-explorer-mock

~~~
Cryptokami SL Explorer web mock.

Usage: cryptokami-explorer-mock [--version]
...
## cryptokami-addr-convert

~~~
Tool to convert vending addresses into testnet addresses.

Usage: cryptokami-addr-convert [--version] [-a|--address STRING]
...
Command example:
  ...
  stack exec -- cryptokami-addr-convert -a 2HF83bvYCTzoCbVta6t64W8rFEnvnkJbIUFoT5tOyoU=
...
## cryptokami-blockchain-analyser

~~~
Cryptokami SL blockchain generator

Usage: cryptokami-blockchain-analyser --db FILEPATH ([-k] | [-m] | [-g] | [-a] |
...
Command example:
  ...
  cryptokami-blockchain-analyser --db /path/to/existing/db
...
Usage: cryptokami-auxx [--version] COMMAND [--db-path FILEPATH] [--rebuild-db]
                    [--genesis-secret INT] [--keyfile FILEPATH]
                    [--backup-phrase PHRASE] [--topology FILEPATH]
                    [--kademlia FILEPATH] [--node-id NODE_ID]
                    [--default-port PORT] [--policies FILEPATH]
...
~~~
Tool to generate CLI-docs for Cryptokami SL executable files.

Usage: cryptokami-cli-docs [--version] --bin-dir PATH
  Generate Markdown chapter for cryptokamidocs.com.

Available options:
  -h,--help                Show this help text
  --version                Show version.
  --bin-dir PATH           Path to the directory with Cryptokami SL executable
                           files.

Assumed that this program will run on Travis CI. Produced file
'cryptokami-cli-docs.md' will be renamed in a chapter and pushed in cryptokamidocs.com
repository.
~~~

## cryptokami-genupdate

~~~
Cryptokami SL updates generator.

Usage: cryptokami-genupdate [--version] --old PATH --new PATH (-o|--output PATH)
...
Command example:
  ...
  stack exec -- cryptokami-genupdate --old /tmp/app-v000 --new /tmp/app-v001 -o /tmp/app-update.tar

Both directories must have equal file structure (e.g. they must contain the same
files in the same subdirectories correspondingly), otherwise 'cryptokami-genupdate' will fail.

Please note that 'cryptokami-genupdate' uses 'bsdiff' program, so make sure 'bsdiff' is available in the PATH.
...
Usage: cryptokami-explorer [--version] [--db-path FILEPATH] [--rebuild-db]
                        [--genesis-secret INT] [--keyfile FILEPATH]
                        [--backup-phrase PHRASE] [--topology FILEPATH]
                        [--kademlia FILEPATH] [--node-id NODE_ID]
                        [--default-port PORT] [--policies FILEPATH]
                        ...
                        [--notifier-port PORT]
  Cryptokami SL main server node w/ explorer.
...
~~~
Cryptokami SL Wallet web API docs generator.

Usage: cryptokami-swagger [--version]
...
## cryptokami-post-mortem

~~~
cryptokami-post-mortem

Usage: cryptokami-post-mortem COMMAND
